...
<style>
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    .header-sub { color: #666; font-size: 1rem; margin-top: 0.4rem; }
    .tags { display: flex; justify-content: center; flex-wrap: wrap; gap: 0.5rem; margin: 1.5rem 0 2.5rem 0; }
    .tag {
        background: rgba(124, 111, 255, 0.1); border: 1px solid rgba(124, 111, 255, 0.3);
        color: #9d91ff; padding: 0.3rem 0.9rem; border-radius: 999px;
        font-size: 0.72rem; font-weight: 600; letter-spacing: 0.04em;
    }
    .stTextInput > div > div > input {
        background: #1a1a1a !important; border: 1px solid #2a2a2a !important;
        border-radius: 14px !important; color: #f0f0f0 !important;
        padding: 0.9rem 1.2rem !important; font-size: 1rem !important;
    }
    ...
    .answer-label {
        ...
        text-transform: uppercase; color: #48cfad; margin-bottom: 0.8rem;
    }
    .answer-text { font-size: 1rem; color: #e8e8e8; line-height: 1.8; }
    .metrics-row { display: flex; gap: 1rem; margin: 1.2rem 0; }
    .metric-box {
        flex: 1; background: #1a1a1a; border: 1px solid #2a2a2a;
        border-radius: 14px; padding: 1rem; text-align: center;
    }
    .metric-value { font-size: 1.6rem; font-weight: 800; color: #7c6fff; }
    .metric-label { font-size: 0.68rem; color: #555; text-transform: uppercase; letter-spacing: 0.08em; margin-top: 0.2rem; }
    .chunk-card {
        background: #141414; border: 1px solid #222;
        border-left: 3px solid #7c6fff; border-radius: 12px;
        padding: 1.1rem 1.3rem; margin-bottom: 0.8rem;
        font-size: 0.86rem; color: #999; line-height: 1.65;
    }
    .chunk-header {
        font-size: 0.68rem; font-weight: 700; color: #7c6fff;
    ...
    .history-q { font-size: 0.82rem; color: #7c6fff; font-weight: 600; margin-bottom: 0.3rem; }
    .history-a { font-size: 0.85rem; color: #888; line-height: 1.6; }
...
def auto_ingest():
    ...
    possible_paths = [
        "docs/ai methodology.pdf",
        "/app/docs/ai methodology.pdf",
        os.path.join(os.path.dirname(__file__), "docs", "ai methodology.pdf")
    ]
    ...
    if pdf_path is None:
        st.warning("PDF not found. Searched in: " + str(possible_paths))
        return
...
def retrieve_and_answer(question):
    ...
    if not all_chunks:
        return "No documents found in knowledge base.", [], [], 0
